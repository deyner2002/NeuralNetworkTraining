...
datos_entrenamiento = (
    "C:/Users/solan/OneDrive/Documentos/Decimo semestre/Proyecto Grado/Proyecto/Proyecto/Fotos/Entrenamiento"
)
datos_validacion = (
    "C:/Users/solan/OneDrive/Documentos/Decimo semestre/Proyecto Grado/Proyecto/Proyecto/Fotos/Validacion"
)
...
preprocesamiento_entre = ImageDataGenerator(
    rescale=1.0 / 255,  # Pasar los pixeles de 0 a 255 | 0 a 1
...
preprocesamiento_vali = ImageDataGenerator(rescale=1.0 / 255)
...
cnn.add(
    Convolution2D( filtrosconv1,tam_filtro1, padding="same", input_shape=(altura, longitud, 3), activation="relu",)
)  # Agregamos la primera capa
# Es una convolucion y realizamos config
cnn.add(
    MaxPooling2D(pool_size=tam_pool)
)  # Despues de la primera capa vamos a tener una capa de max pooling y asignamos el tamaño

cnn.add(
    Convolution2D(filtrosconv2, tam_filtro2, padding="same", activation="relu")
)  # Agregamos nueva capa

# cnn.add(
#     Convolution2D(filtrosconv3, tam_filtro3, padding="same", activation="relu")
# )  # Agregamos nueva capa 
...
# Obtener datos de precisión y pérdida durante el entrenamiento
precision = historia.history['accuracy']
val_precision = historia.history['val_accuracy']
